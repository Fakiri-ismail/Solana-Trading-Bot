...
for token in wallet_assets:
    ...
    if token["mint"] not in db_mints:
        ...
        threshold_upper = 1.8 * db_token.purchase_price
        threshold_lower = 0.5 * db_token.purchase_price

        if token_actual_price >= threshold_upper or token_actual_price <= threshold_lower:
            my_wallet.sell_token(mint=token["mint"], pct_amount=100)
            create_trading_history(
                mint=token["mint"],
                symbol=token["symbol"], 
                usdt_value=token_usd_value,
                sale_price=token_actual_price
            )
